...
function App() {
	...
	return (
		<div className="App">
			...
			<ChuckCard title="Chuck Norris" />

			<h2>Chuck Info: </h2>
			<ChuckInfo key={roundHouseKicks} whalesSaved={whalesSaved} roundHouseKicks={roundHouseKicks} />

			<h2>Jokes: </h2>
			{jokes.map((joke) => <ChuckJoke key={joke.id} id={joke.id} joke={joke.joke} />)}

			<h2>Filtered Jokes: </h2>
			{filteredJokes.map((joke) => <ChuckJoke key={joke.id} id={joke.id} joke={joke.joke} />)}
